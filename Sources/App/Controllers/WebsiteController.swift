...
struct WebsiteController: RouteCollection {
    ...
    func boot(router: Router) throws {
        ...
        authSessionRoutes.post("categories", Category.parameter, "delete", use: deleteCategoryHandler)
                    ...
                    .flatMap(to: Response.self) { savedAcronym in
                        ...
                        return try acronym.categories.query(on: req).all()
                            .flatMap(to: Response.self, { existingCategories in
                                ...
                                var categoriesResults: [Future<Void>] = []
                                for newCategory in categoriesToAdd {
                                    categoriesResults.append(try Category.addCategory(newCategory, to: acronym, on: req))
                                }
                                
                                for categoryNameToRemove in categoriesToRemove {
                                    let categoryToRemove = existingCategories.first { $0.name == categoryNameToRemove }
                                    if let category = categoryToRemove {
                                        categoriesResults.append(acronym.categories.detach(category, on: req))
                                    }
                                }
    ...
    func deleteCategoryHandler(_ req: Request) throws
        -> Future<Response> {
    ...
    func loginPostHandler(_ req: Request, userData: LoginPostData) -> Future<Response> {
